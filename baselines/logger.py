...
class TensorBoardOutputFormat(KVWriter):
    ...
    def __init__(self, dir):
        os.makedirs(dir, exist_ok=True, mode=755)
        self.dir = dir
        self.step = 1
        prefix = 'events'
...
def make_output_format(format, ev_dir):
    os.makedirs(ev_dir, exist_ok=True, mode=755)
...
def configure(dir=None, format_strs=None,theta = 0):
    if dir is None:
        dir = os.getenv('OPENAI_LOGDIR')
    if dir is None:
        dir = osp.join('./LOG/',datetime.datetime.now().strftime("openai-%Y-%m-%d-%H-%M-%S-%f"))
